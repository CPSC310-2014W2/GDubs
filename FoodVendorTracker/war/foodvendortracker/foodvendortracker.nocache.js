...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "foodvendortracker" because\n';
  ...
  $wnd.__gwt_activeModules['foodvendortracker'] = {
    'moduleName' : 'foodvendortracker',
    'bindings' : function() {
      return {};
    }
  };
 ...
 * "/foodvendortracker.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['foodvendortracker'].superdevmode = true;

  var expectedSuffix = '/foodvendortracker.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + foodvendortracker + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:foodvendortracker';
  var devModeSessionKey = '__gwtDevModeSession:foodvendortracker';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/foodvendortracker/foodvendortracker.nocache.js';
  ...
  $wnd.__gwt_activeModules['foodvendortracker'].canRedirect = true;
  $wnd.__gwt_activeModules['foodvendortracker'].superdevmode = true;
  ...
  css = (moduleIdx == 1 ? css : '') +
    ".gwt-DevModeModule-" + moduleIdx + ".gwt-DevModeCompiling div:before{" +
      "content:'COMPILING foodvendortracker';" +
      "font-size:24px;" +
      "color:#d2d9ee;" +
    "}";
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/foodvendortracker?user.agent=' + ua + '&_callback=' + callback;
